...
const Entry: FunctionComponent<{ dict: any }> = ({ dict }) => {
  return (
    <div className="w-full flex flex-col items-center justify-center relative gap-5">
      <div className="relative w-full flex flex-col">
        <div className="relative w-full h-[45rem] flex flex-row justify-between items-start gap-8">
          <Futures dict={dict} />
            ...
            {Array.from({ length: 30 }).map((_, index: number) => {
              return (
                <span className="relative text-sm px-5" key={index}>
                  LIQUID FASHION FUTURES ** BOT SETTLEMENT ** AMM LP **
                  ESCROW PHYSICAL RIGHTS ** SPECTATE FULFILLMENT **
                </span>
              );
            })}
          </Marquee>
