...
const Fulfillers: FunctionComponent<{dict: any}> = ({ dict }) => {
  const { fulfillers, fulfillersLoading, hasMoreFulfillers, loadMoreFulfillers } = useFulfillers();
  ...
  if (fulfillersLoading && fulfillers?.length === 0) {
    return (
      <div className="w-full flex flex-col p-6">
        <div className="text-2xl font-bold mb-6 text-left">Spectate Fulfillers</div>
        <div className="text-center text-gray-500 py-8">Loading fulfillers...</div>
      </div>
    );
  }
    ...
    <div className="w-full flex flex-col p-6">
      ...
      >
        ...
        >
          ...
          <div key={fulfiller.fulfillerId} className="bg-white border border-black p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <div className="font-mono text-xs">
                        ...
                        >
                          {fulfiller.fulfiller.slice(0, 8)}...{fulfiller.fulfiller.slice(-6)}
                        </a>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="font-medium">{fulfiller.infraId}</div>
                    </div>
                    
                    <div>
                      <span className="text-gray-600">Active Orders:</span>
                      <div className="font-medium">{fulfiller.childOrders?.length || 0}</div>
                    </div>
                  </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {fulfiller.childOrders?.map((order: ChildOrder, orderIndex: number) => {
                  const isExpanded = expandedOrders[`${fulfiller.fulfillerId}-${orderIndex}`];
                  
                  return (
                    <div key={orderIndex} className="border border-black bg-white">
                      <div 
                        className="p-3 cursor-pointer hover:bg-gray-100 transition-colors"
                        onClick={() => toggleOrderExpanded(fulfiller.fulfillerId, orderIndex)}
                      >
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 flex-shrink-0 relative">
                              <Image
                                ...
                                fill
                                style={{ objectFit: "cover" }}
                                src={order.parent.metadata.image}
                            ...
                            <div>
                              ...
                              <div className="space-y-2 text-sm">
                                <div className="grid grid-cols-2 gap-2">
                                  ...
                                  <span className="font-mono text-xs">{order.parent.designId}</span>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  ...
                                  <div className="font-mono text-xs">
                                    <a
                                      ...
                                      className="text-blue-600 hover:text-blue-800 underline"
                                    >
                                      {order.parent.parentContract.slice(0, 8)}...{order.parent.parentContract.slice(-6)}
                                    </a>
                                  </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  ...
                                  <span>{new Date(parseInt(order.fulfillment.createdAt)).toLocaleDateString()}</span>
                                </div>
                                
                                <div className="grid grid-cols-2 gap-2">
                                  <span className="text-gray-600">Last Updated:</span>
                                  <span>{new Date(parseInt(order.fulfillment.lastUpdated)).toLocaleDateString()}</span>
                                </div>
                              </div>
                              <div className="mt-4">
                            ...
                            <div>
                              ...
                              <div className="border border-black bg-white p-3">
                                ...
                                <div className="flex items-start gap-2">
                                  <div className="w-10 h-10 flex-shrink-0 relative">
                                    <Image
                                      ...
                                      fill
                                      style={{ objectFit: "cover" }}
                                      src={order.parent.workflow.physicalSteps.fulfiller.metadata.image}
                                      alt={order.parent.workflow.physicalSteps.fulfiller.metadata.title}
                                      className="border border-gray-200"
                                    />
                                  </div>
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="text-xs text-gray-600 font-mono">
                                      <a
                                        ...
                                        className="text-blue-600 hover:text-blue-800 underline"
                                      >
                                        {order.parent.workflow.physicalSteps.fulfiller.fulfiller.slice(0, 8)}...
                                        {order.parent.workflow.physicalSteps.fulfiller.fulfiller.slice(-6)}
                                      </a>
                                    </div>
                                    <div className="text-xs text-gray-600 mt-1">
                                      Infrastructure: {order.parent.workflow.physicalSteps.fulfiller.infraId}
                                    </div>
                                    <a
                                      href={order.parent.workflow.physicalSteps.fulfiller.metadata.link}
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="text-xs text-blue-600 hover:text-blue-800 underline"
                                    >
                                ...
                                <div className="mt-2 text-xs text-gray-600">
                                  <span className="font-medium">Revenue Split:</span> {(Number(order.parent.workflow.physicalSteps.subPerformers.splitBasisPoints) / 100)}%
                                </div>
                              </div>
              ...
              {(!fulfiller.childOrders || fulfiller.childOrders.length === 0) && (
